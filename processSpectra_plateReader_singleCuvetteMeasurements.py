...
# SPECIFICALLY for when you have multiple individual cuvette readings.
...
class Spectrum:
	def __init__(self, name, time, date, wavelengths,absorbances):
		...
		hour = float(self.time[-12:-10])
		minute = float(self.time[-9:-7])
		second = float(self.time[-6:-4])
...
def readSpectrumFile(fileName):
	# this first bit is hard coded. i rely on the file having one lines, then a line with date and time, then five blank lines,
	...
	# when you read in cuvette readings, you get one line for each wavelength.
	thisLine = thisFile.readline()
	while len(thisLine)>5:
		entries = thisLine.split(",")
		wavelengths_thisFile.append(float(entries[1]))
		absorbances_thisFile.append(float(entries[2]))
		thisLine = thisFile.readline()
